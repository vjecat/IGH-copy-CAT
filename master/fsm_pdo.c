...
void ec_fsm_pdo_i950_state_pdo1(ec_fsm_pdo_t *, ec_datagram_t *);
void ec_fsm_pdo_i950_state_pdo2(ec_fsm_pdo_t *, ec_datagram_t *);
void ec_fsm_pdo_i950_state_pdo3(ec_fsm_pdo_t *, ec_datagram_t *);
void ec_fsm_pdo_i950_state_pdo4(ec_fsm_pdo_t *, ec_datagram_t *);
void ec_fsm_pdo_i950_state_pdo5(ec_fsm_pdo_t *, ec_datagram_t *);
void ec_fsm_pdo_i950_state_pdo6(ec_fsm_pdo_t *, ec_datagram_t *);
void ec_fsm_pdo_i950_state_pdo7(ec_fsm_pdo_t *, ec_datagram_t *);
void ec_fsm_pdo_i950_state_pdo8(ec_fsm_pdo_t *, ec_datagram_t *);
...
/** Start writing the PDO configuration. ... by luigi
 */
void ec_fsm_pdo_i950_settings(
        ec_fsm_pdo_t *fsm, /**< PDO configuration state machine. */
        ec_slave_t *slave, /**< slave to configure */
        ...
        )
{
	...
	// fsm->state = ec_fsm_pdo_state_end;	// exit
	
	// ec_pdo_init(&fsm->slave_pdo);
    // if (ec_sdo_request_alloc(&fsm->request, 16)) {
    //    fsm->state = ec_fsm_pdo_state_error;
    //    return;
    // }
	// fsm->pdo->index = 0x1608;	// EC_READ_U16(fsm->request.data);
    // fsm->pdo->sync_index = 2;	// fsm->sync_index;
    // list_add_tail(&fsm->pdo->list, &fsm->pdos.list);
	...
	// assign first PDO
	fsm->pdo->index = 0x1608;	// index
    fsm->pdo_pos = 1;		// subindex
    fsm->sync_index = 2;   // SM2
    
    // PREPISANO= ec_fsm_pdo_conf_action_assign_pdo(fsm, datagram);
    ...
    fsm->state = ec_fsm_pdo_i950_state_pdo1;
...
/** Set next PDO . by luigi
 */
void ec_fsm_pdo_i950_state_pdo1(
        ...
        )
{
    ...
    if (!ec_fsm_coe_success(fsm->fsm_coe)) {
        EC_SLAVE_WARN(fsm->slave, "Failed to set PDO-1 \n");
        EC_SLAVE_WARN(fsm->slave, "");
        // ec_fsm_pdo_print(fsm);
        return;
    }

	EC_SLAVE_DBG(fsm->slave, 1, "Set PDO-2 \n");
	
	// ide 0x1c13,0 = 0
    EC_WRITE_U8(fsm->request.data, 0);
    fsm->request.data_size = 1;
    ecrt_sdo_request_index(&fsm->request, 0x1C13, 0);
    ecrt_sdo_request_write(&fsm->request);

	fsm->state = ec_fsm_pdo_i950_state_pdo2;	// nastavi po redu
    ec_fsm_coe_transfer(fsm->fsm_coe, fsm->slave, &fsm->request);
    ec_fsm_coe_exec(fsm->fsm_coe, datagram); // execute immediately
}
...
/** Set next PDO . by luigi
 */
void ec_fsm_pdo_i950_state_pdo2(
        ec_fsm_pdo_t *fsm, /**< Finite state machine. */
        ec_datagram_t *datagram /**< Datagram to use. */
        )
{
    ...
    if (!ec_fsm_coe_success(fsm->fsm_coe)) {
        EC_SLAVE_WARN(fsm->slave, "Failed to set PDO-2 \n");
        EC_SLAVE_WARN(fsm->slave, "");
        ec_fsm_pdo_print(fsm);
        return;
    }

	EC_SLAVE_DBG(fsm->slave, 1, "Set PDO-3 \n");
  
	// ide 0x1a05,0 = 0
    EC_WRITE_U8(fsm->request.data, 0);
    fsm->request.data_size = 1;
    ecrt_sdo_request_index(&fsm->request, 0x1a05, 0);
    ecrt_sdo_request_write(&fsm->request);

	fsm->state = ec_fsm_pdo_i950_state_pdo3;	// nastavi po redu
    ec_fsm_coe_transfer(fsm->fsm_coe, fsm->slave, &fsm->request);
    ec_fsm_coe_exec(fsm->fsm_coe, datagram); // execute immediately
}
...
/** Set next PDO . by luigi
 */
void ec_fsm_pdo_i950_state_pdo3(
        ec_fsm_pdo_t *fsm, /**< Finite state machine. */
        ec_datagram_t *datagram /**< Datagram to use. */
        )
{
    ...
    if (!ec_fsm_coe_success(fsm->fsm_coe)) {
        EC_SLAVE_WARN(fsm->slave, "Failed to set PDO-3 \n");
        EC_SLAVE_WARN(fsm->slave, "");
        ec_fsm_pdo_print(fsm);
        return;
    }

	EC_SLAVE_DBG(fsm->slave, 1, "Set PDO-4 \n");
  
	// ide 0x1605,0 = 0
    EC_WRITE_U8(fsm->request.data, 0);
    fsm->request.data_size = 1;
    ecrt_sdo_request_index(&fsm->request, 0x1605, 0);
    ecrt_sdo_request_write(&fsm->request);

	fsm->state = ec_fsm_pdo_i950_state_pdo4;	// nastavi po redu
    ec_fsm_coe_transfer(fsm->fsm_coe, fsm->slave, &fsm->request);
    ec_fsm_coe_exec(fsm->fsm_coe, datagram); // execute immediately
}
...
/** Set next PDO . by luigi
 */
void ec_fsm_pdo_i950_state_pdo4(
        ec_fsm_pdo_t *fsm, /**< Finite state machine. */
        ec_datagram_t *datagram /**< Datagram to use. */
        )
{
    ...
    if (!ec_fsm_coe_success(fsm->fsm_coe)) {
        EC_SLAVE_WARN(fsm->slave, "Failed to set PDO-4 \n");
        EC_SLAVE_WARN(fsm->slave, "");
        ec_fsm_pdo_print(fsm);
        return;
    }

	EC_SLAVE_DBG(fsm->slave, 1, "Set PDO-5 \n");
  
	// ide 0x1c12,1 = 0x1608
    EC_WRITE_U16(fsm->request.data, 0x1608);
    fsm->request.data_size = 2;
    ecrt_sdo_request_index(&fsm->request, 0x1c12, 1);
    ecrt_sdo_request_write(&fsm->request);

	fsm->state = ec_fsm_pdo_i950_state_pdo5;	// nastavi po redu
    ec_fsm_coe_transfer(fsm->fsm_coe, fsm->slave, &fsm->request);
    ec_fsm_coe_exec(fsm->fsm_coe, datagram); // execute immediately
}
...
/** Set next PDO . by luigi
 */
void ec_fsm_pdo_i950_state_pdo5(
        ec_fsm_pdo_t *fsm, /**< Finite state machine. */
        ec_datagram_t *datagram /**< Datagram to use. */
        )
{
    ...
    if (!ec_fsm_coe_success(fsm->fsm_coe)) {
        EC_SLAVE_WARN(fsm->slave, "Failed to set PDO-5 \n");
        EC_SLAVE_WARN(fsm->slave, "");
        ec_fsm_pdo_print(fsm);
        return;
    }

	EC_SLAVE_DBG(fsm->slave, 1, "Set PDO-6 \n");
  
	// ide 0x1c12,0 = 0x01
    EC_WRITE_U8(fsm->request.data, 0x01);
    fsm->request.data_size = 1;
    ecrt_sdo_request_index(&fsm->request, 0x1c12, 0);
    ecrt_sdo_request_write(&fsm->request);

	fsm->state = ec_fsm_pdo_i950_state_pdo6;	// nastavi po redu
    ec_fsm_coe_transfer(fsm->fsm_coe, fsm->slave, &fsm->request);
    ec_fsm_coe_exec(fsm->fsm_coe, datagram); // execute immediately
}
...
/** Set next PDO . by luigi
 */
void ec_fsm_pdo_i950_state_pdo6(
        ec_fsm_pdo_t *fsm, /**< Finite state machine. */
        ec_datagram_t *datagram /**< Datagram to use. */
        )
{
    ...
    if (!ec_fsm_coe_success(fsm->fsm_coe)) {
        EC_SLAVE_WARN(fsm->slave, "Failed to set PDO-6 \n");
        EC_SLAVE_WARN(fsm->slave, "");
        ec_fsm_pdo_print(fsm);
        return;
    }

	EC_SLAVE_DBG(fsm->slave, 1, "Set PDO-7 \n");
  
	// ide 0x1c13,1 = 0x1a08
    EC_WRITE_U16(fsm->request.data, 0x1a08);
    fsm->request.data_size = 2;
    ecrt_sdo_request_index(&fsm->request, 0x1c13, 1);
    ecrt_sdo_request_write(&fsm->request);

	fsm->state = ec_fsm_pdo_i950_state_pdo7;	// nastavi po redu
    ec_fsm_coe_transfer(fsm->fsm_coe, fsm->slave, &fsm->request);
    ec_fsm_coe_exec(fsm->fsm_coe, datagram); // execute immediately
}
...
/** Set next PDO . by luigi
 */
void ec_fsm_pdo_i950_state_pdo7(
        ec_fsm_pdo_t *fsm, /**< Finite state machine. */
        ec_datagram_t *datagram /**< Datagram to use. */
        )
{
    ...
    if (!ec_fsm_coe_success(fsm->fsm_coe)) {
        EC_SLAVE_WARN(fsm->slave, "Failed to set PDO-7 \n");
        EC_SLAVE_WARN(fsm->slave, "");
        ec_fsm_pdo_print(fsm);
        return;
    }

	EC_SLAVE_DBG(fsm->slave, 1, "Set PDO-8 \n");
	...
	fsm->state = ec_fsm_pdo_i950_state_pdo8;	// nastavi po redu
    ec_fsm_coe_transfer(fsm->fsm_coe, fsm->slave, &fsm->request);
    ec_fsm_coe_exec(fsm->fsm_coe, datagram); // execute immediately
}
...
/** Set next PDO . by luigi
 */
void ec_fsm_pdo_i950_state_pdo8(
        ec_fsm_pdo_t *fsm, /**< Finite state machine. */
        ec_datagram_t *datagram /**< Datagram to use. */
        )
{
    ...
    if (!ec_fsm_coe_success(fsm->fsm_coe)) {
        EC_SLAVE_WARN(fsm->slave, "Failed to set PDO-8 \n");
        EC_SLAVE_WARN(fsm->slave, "");
        ec_fsm_pdo_print(fsm);
        return;
    }

	EC_SLAVE_DBG(fsm->slave, 1, "PDO assigned ... return \n");
...
{
	// EC_LUIGI_DBG(fsm->slave, 1, "proc= %s\n", __func__);
        ...
        )
{
    ...
    if (fsm->slave->sii.product_code == 0x19004014)
    {
		fsm->pdo_count = 0x01;		/** luigi  */
	}
	else {
        ...
        )
{
    ...
    {
    	/** luigi  */
		if (fsm->pdo->index > 0x1700) {
			fsm->pdo->index = 0x1a08;		/**<  0x1a00 >>> 0x1a08  */
		}
		else {
			fsm->pdo->index = 0x1608;		/**<  0x1600 >>> 0x1608  */
		}
	}	// if lenze i950
