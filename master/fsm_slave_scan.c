...
void ec_fsm_slave_scan_state_i950(ec_fsm_slave_scan_t *);	/** luigi */
void ec_fsm_slave_scan_state_i950_2(ec_fsm_slave_scan_t *);	/** luigi */
void ec_fsm_slave_scan_state_i950_3(ec_fsm_slave_scan_t *);	/** luigi */
void ec_fsm_slave_scan_state_i950_4(ec_fsm_slave_scan_t *);	/** luigi */
...
{	
    ...
    ec_fsm_i950_init(&fsm->fsm_i950, fsm->datagram);	/**< luigi */
}
...
void ec_fsm_slave_scan_clear(ec_fsm_slave_scan_t *fsm /**< slave state machine */)
{
    ...
    ec_fsm_i950_clear(&fsm->fsm_i950);
}
...
/** Enter slave scan state PDOS. by luigi
 */
...
{   
	...
	EC_LUIGI_DBG(fsm->slave, 1, "proc= %s\n", __func__);
	// EC_SLAVE_DBG(slave, 1, "Enter TWINCAT startup by luigi \n");
    // fsm->state = ec_fsm_slave_scan_state_i950;
    // ec_fsm_i950_start(&fsm->fsm_i950, slave);
    // ec_fsm_i950_exec(&fsm->fsm_i950);
...
/** Slave scan state: LENZE i950 STARTUP.  by luigi
 */
void ec_fsm_slave_scan_state_i950(
        ec_fsm_slave_scan_t *fsm /**< slave state machine */
        )
{
    if (ec_fsm_i950_exec(&fsm->fsm_i950)) {
        return;
    }

    if (!ec_fsm_i950_success(&fsm->fsm_i950)) {
        fsm->state = ec_fsm_slave_scan_state_error;
        return;
    }

    // startup i950 continued
    fsm->state = ec_fsm_slave_scan_state_i950_2;
}

/*****************************************************************************/

/** Enter slave scan state nastavi. by luigi
 */
void ec_fsm_slave_scan_state_i950_2(
        ec_fsm_slave_scan_t *fsm /**< slave state machine */
        )
{   
	ec_slave_t *slave = fsm->slave;
	
	EC_SLAVE_DBG(slave, 1, "TWINCAT nastavak by luigi \n");
	
	

    fsm->state = ec_fsm_slave_scan_state_i950_3;
    ec_fsm_pdo_i950_settings(fsm->fsm_pdo, slave, fsm->datagram);
    ec_fsm_pdo_exec(fsm->fsm_pdo, fsm->datagram); // execute immediately  
}

/*****************************************************************************/

/** Slave scan state: LENZE i950 nastavak.  by luigi
 */
void ec_fsm_slave_scan_state_i950_3(
        ec_fsm_slave_scan_t *fsm /**< slave state machine */
        )
{
	ec_slave_t *slave = fsm->slave;
    ...
    // startup i950 continued
    EC_SLAVE_DBG(slave, 1, "TWINCAT treci dio by luigi \n");
    fsm->state = ec_fsm_slave_scan_state_i950_4;
    ec_fsm_i950_enter(&fsm->fsm_i950, slave);
    ec_fsm_i950_exec(&fsm->fsm_i950);
}
...
/** Slave scan state: LENZE i950 STARTUP.  by luigi
 */
void ec_fsm_slave_scan_state_i950_4(
        ec_fsm_slave_scan_t *fsm /**< slave state machine */
        )
{
    if (ec_fsm_i950_exec(&fsm->fsm_i950)) {
    ...
    fsm->state = ec_fsm_slave_scan_state_end;	// exit
}
